...
import {
  convertFetchedLinksToOneDirectional,
  convertLinksFromFetchedToMultiselectFormat,
  removeNameData,
} from "./JarjestelmaLinkUtils.js";
...
export default class JarjestelmaSalkku extends Component {
  ...
  refresh(tunnus) {
    if (tunnus) {
      this.props.jarjestelmaSalkkuStore.fetchDetails(tunnus).then(() => {
        ...
        const listWithoutNameData = removeNameData(
          this.props.jarjestelmaSalkkuStore.details.jarjestelmaLinkkausList
        );
        const oneDirectional = convertFetchedLinksToOneDirectional(
          listWithoutNameData,
          Number(tunnus)
        );
        delete this.props.jarjestelmaSalkkuStore.details
          .jarjestelmaLinkkausList;
        const [multiLinks, metadata] =
          convertLinksFromFetchedToMultiselectFormat(oneDirectional);

        this.props.jarjestelmaSalkkuStore.details.jarjestelmaLinkkausListMultiselect =
          multiLinks;
        this.props.jarjestelmaSalkkuStore.details.jarjestelmaLinkkausMetadata =
          metadata;
        this.props.jarjestelmaSalkkuStore.details = toJS(
    ...
    this.props.jarjestelmaSalkkuStore.fetchRights();
  }
  ...
  render() {
    ...
    if (this.jarjestelma) {
      ...
      const emptyValuesWithRights = {
        noRightsToModify: this.props.jarjestelmaSalkkuStore.noRightsToModify,
      };
      contentEl = (
        <JarjestelmaForm
          values={emptyValuesWithRights}
          resources={this.props.jarjestelmaSalkkuStore.resources}
          edit={edit}
          onSubmit={(values) => {
            this.validateBeforeSubmit(values);
